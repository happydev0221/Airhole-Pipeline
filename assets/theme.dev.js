...
(function (bodyScrollLock, MicroModal, Rellax, Flickity, themeCurrency, Sqrl, themeAddresses, axios, FlickityFade, Poppy, ellipsed, FlickitySync, AOS) {
  ...
  class Bar {
    ...
    initTickers(stopClone = false) {
      const frames = this.barHolder.querySelectorAll(selectors$H.frame);
  ...
  class HoverDisclosure {
    ...
    showDisclosure() {
      ...
      this.transitionTimeout = setTimeout(function () {
        instance.wrapper.classList.remove(classes$f.meganavIsTransitioning);
      }, 200);
    }
  ...
  class Media {
    ...
    createSlider() {
      ...
      let flickityOptions = {
        ...
        wrapAround: true,
        fade: true,
        cellSelector: '.product__media:not(.hide)',
        on: {
          ready: function () {
      ...
      var showHideImages = function(variant, allImages) {
        var matchFound = false;
        for (var i=0; i<allImages.length; i++) {
          var image = allImages[i];
          var match = false;
          for (var j=0; j<variant.options.length; j++) {
            if (image.dataset.alt === variant.options[j]) {
              match = true;
            }
          }
          if (match) {
      ...
      var showImages = function(allImages) {
        for (var i=0; i<allImages.length; i++) {
          var image = allImages[i];
          image.classList.remove("hide");
        }
      ...
      document.addEventListener('theme:variant:change', function(event) {        
        ...
        var matchFound = false;
        if (event && event.detail && event.detail.variant && event.detail.variant.options) {
          matchFound = showHideImages(event.detail.variant, document.querySelectorAll('[data-product-slideshow] .product__media'));          
          showHideImages(event.detail.variant, document.querySelectorAll('.media__thumb'));          
        }
        if (!matchFound) {
          showImages(document.querySelectorAll('[data-product-slideshow] .product__media'));
          showImages(document.querySelectorAll('.media__thumb'));
        }
        this.flkty = new FlickityFade(this.slideshow, flickityOptions);
        this.flkty.resize();
      }.bind(this));
    ...
    sliderThumbs() {
      let opts = {
        ...
        sync: this.slideshow,
        cellSelector: '.media__thumb:not(.hide)',
      };
